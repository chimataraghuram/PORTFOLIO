...
const HeroTexturedPlanet: React.FC<HeroTexturedPlanetProps> = ({ planetId, isMobile = false }) => {
  const cssFallback = (
    <div
      className={`absolute ${isMobile ? 'right-[-5%] top-[30%] h-[65vmin] w-[65vmin]' : 'right-[-8%] top-[50%] h-[75vmin] w-[75vmin]'} -translate-y-1/2 rounded-full opacity-80 transition-all duration-1000`}
      style={{
        background:
          planetId === 'earth'
    ...
    <div className="absolute inset-0 w-full h-full pointer-events-none">
      ...
      {!isMobile && (
        <PlanetErrorBoundary fallback={null}>
          <div className="absolute right-[-8%] top-[50%] h-[75vmin] w-[75vmin] -translate-y-1/2">
            <Canvas
